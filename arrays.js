const { getVarHandler, setVarHandler } = require("./vars_primitive.js");
...
function setArrayIndex(args) {
    ...
    if (value.startsWith("GETVAR->")) {
        let varValue = getVarHandler(value.replace("GETVAR->", ""));
        if (varValue[1] == 1) {
            return 1;
        }
        value = varValue[0];
...
module.exports = { setArray, getArray, setArrayIndex }